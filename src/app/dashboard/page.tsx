...
export default async function DashboardPage({ searchParams }: DashboardPageProps) {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-md flex-col gap-4 px-4 py-6 sm:px-6">
      <header className="mb-5 flex items-start justify-between gap-3">
        <div>
          <p className="text-sm text-muted-foreground">{dashboard.dateLabel}</p>
          <h1 className="mt-1 text-2xl font-semibold tracking-tight">Habit Buckets</h1>
          <Link href="/history" className="mt-1 inline-block text-xs text-muted-foreground underline">
            View history
          </Link>
        </div>
      ...
      <section className="space-y-3 rounded-2xl border bg-card p-4 text-card-foreground shadow-sm">
        ...
        <ul className="space-y-2">
          {dashboard.buckets.map((bucket) => {
            ...
            return (
              <li
                key={bucket.id}
                className={`flex min-h-12 items-center justify-between rounded-xl border px-3 py-2 ${tone.containerClass}`}
              >
      ...
      <section className="mt-4 space-y-3 rounded-2xl border bg-card p-4 text-card-foreground shadow-sm">
        ...
        {dashboard.hasActivities ? (
          <ul className="space-y-2">
            {dashboard.activities.map((activity) => {
              ...
              return (
                <li key={activity.id} className="rounded-xl border px-3 py-2">
                <form action={toggleActivityAction}>
                  <input type="hidden" name="activityId" value={activity.id} />
                  <input type="hidden" name="date" value={todayState.date} />
                  <button
                    type="submit"
                    className="flex min-h-12 w-full items-center justify-between gap-2 rounded-md text-left"
                  >
                    <div>
        ...
        <form action={createActivityAction} className="rounded-xl border p-3">
          ...
          <div className="mt-2 space-y-2">
            ...
            <div className="space-y-2">
              {todayState.buckets.map((bucket) => (
                <label key={`create-${bucket.id}`} className="flex items-center gap-2 text-sm">
          ...
          <div className="space-y-2 rounded-xl border p-3">
            ...
            <ul className="space-y-3">
              {todayState.activities.map((activity) => (
                <li key={`edit-${activity.id}`} className="rounded-lg border p-2">
                  <form action={updateActivityAction} className="space-y-2">
                    ...
                    <div className="space-y-2">
                      {todayState.buckets.map((bucket) => (
                        <label key={`edit-${activity.id}-${bucket.id}`} className="flex items-center gap-2 text-sm">
                          <input
                            className="size-5"
                            type="checkbox"
                            name="bucketIds"
